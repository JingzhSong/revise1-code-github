function [output,cmax] = singularvalue_up(kx,kz,c,N,R,flowtype,method,eddy,wall,Y)
% (kx,kz,c): wave number
% N: y+ grid number
% R: Reynolds number
...
if strcmp(method,'SIOA')
    if isempty(c)
        ...
        Mp = dss(A,B,C,0,E);
        boundsp = mussv(Mp,BlockStructure,'Uf');
        Mn = dss(-A,-B,C,0,E);
        boundsn = mussv(Mn,BlockStructure,'Uf');
        output = max(max(squeeze(boundsp.ResponseData(1,1,:))),max(squeeze(boundsn.ResponseData(1,1,:))));
        cmax = 0;  
    else
        H = C*((-1i*c*kx*E-A)\B);
        BlockStructure = [N,3*N;N,3*N;N,3*N];
        bound = mussv(H,BlockStructure,'Uf');
        output = bound(1,1);
        cmax = 0;
    end
end
